/*
  Telegram Bot - WormGPT-v7
  Developer - DitzzXploit
*/
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    msg.chat.id,
    `👋 Halo *${msg.from.first_name}*!\nSaya adalah AI WormGPT-v7.\n\nKetik pesanmu untuk mulai chat denganku.\n\n💡 Kamu punya *${settings.freeLimit}* limit gratis per hari.\n\n┌───「 NEW FITURE 」\n│ /stalk-tiktok username\n│ /ngl-spam urlngl text jumlah\n└⪩\n\n@ofcwormgpt`,
    {
      reply_markup: {
      inline_keyboard: [
        [
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "buy_limit") {
    bot.sendMessage(ChatId, "Untuk Buy Limit silahkan hubungi Dev\n@cinahitamnih")
  }
});

bot.onText(/\/limit/, async (msg) => {
  const sisa = await getLimit(msg.from.id);
  bot.sendMessage(msg.chat.id, `📊 Sisa limit kamu: *${sisa}*`, { parse_mode: "Markdown" });
});
...
bot.on("message", async (msg) => {
  ...
  if (limit <= 0) {
    return bot.sendMessage(
      msg.chat.id,
      "⚠️ Limit kamu sudah habis!\nTunggu reset harian (00:00) atau hubungi owner.",
      { parse_mode: "Markdown" }
    );
  }
...
console.log("🤖 WormGPT-v7 aktif dan siap digunakan!");
